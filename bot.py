...
# PUT YOUR ORIGINAL ADMIN ID HERE:
    ...
    # --- PROCESS OLD GMAIL APPROVALS/REJECTIONS ---
        ...
        if data == "wd_cancel":
            ...
            if wd_type == 'MANUAL':
                ...
                full_webapp_url = f"https://mini-app-3-seven.vercel.app?{query_string}"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # We strictly POP the state so it can never trigger multiple times or get stuck
    ...
    try:
        if state == 'ADM_PRIZE_SETUP' and user_id in ADMIN_IDS:
            ...
            task_match = await db_query("SELECT id FROM tasks WHERE task_data LIKE ? OR task_data = ?", (f"{text}:%", text), fetchone=True)
            if task_match:
                await db_query("UPDATE users SET is_banned=1 WHERE user_id=?", (user_id,), commit=True)
                try: await update.message.reply_text("🚫 you are banned for selling task mails in old mails for more info contact support")
                except: pass
                
                adm_msg = f"🚨 **AUTO-BAN ALERT**\n\nUser ID `{user_id}` banned for selling task mails in old mails."
                for admin in ADMIN_IDS:
                    try: await context.bot.send_message(admin, adm_msg, parse_mode="Markdown")
                    except: pass
                return
        ...
        elif state == 'ADM_VERIFY_USER' and user_id in ADMIN_IDS:
            if text.isdigit():
                target_id = int(text)
            ...
            uid_str = text.strip().lstrip('-')
            if not uid_str.isdigit():
                await update.message.reply_text("❌ Invalid ID. Please enter a valid numeric User ID.")
                return
            target_id = int(text.strip())
            
            target_user = await db_query("SELECT user_id FROM users WHERE user_id = ?", (target_id,), fetchone=True)
            if not target_user:
            ...
            b = await db_query("SELECT balance FROM users WHERE user_id=?", (target_id,), fetchone=True)
            if not b:
                await update.message.reply_text(f"❌ User ID {target_id} not found in database.")
                return
            await update.message.reply_text(f"Bal: ₹{b[0]}")
            
        elif state == 'ADM_MOD_BAL' and user_id in ADMIN_IDS:
            if ":" in text:
                try:
                    target_str, amt_str = text.split(":", 1)
                    target = int(target_str.strip())
                    amt = float(amt_str.strip())
                    
                    target_user = await db_query("SELECT user_id FROM users WHERE user_id = ?", (target,), fetchone=True)
                    if not target_user:
                        await update.message.reply_text(f"❌ User ID {target} not found in database.")
                        return
                        
                    await db_query("UPDATE users SET balance=balance+? WHERE user_id=?", (amt, target), commit=True)
                    await update.message.reply_text(f"✅ Balance for User {target} successfully updated.")
                except ValueError:
                    await update.message.reply_text("❌ Invalid format. Make sure you entered numeric values.")
            else:
                await update.message.reply_text("❌ Invalid Format. Use `id:amt`")
